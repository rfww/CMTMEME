...
class BertImageFeatureEmbeddings(nn.Module):
    ...
    def forward(self, image_feature: Tensor) -> Tensor:
...
class ViLBERTBase(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.resnet152=resnet.resnet152(pretrained=True)
    ...
    def forward(
        ...
        image_feature = self.resnet152(image_feature)
        ...
        v_embedding_output = self.v_embeddings(image_feature)
    ...
    def forward(
        ...
        if image_label is not None :
            if self.visual_target == 1:
                ...
                img_loss = self.vis_criterion(prediction_scores_v, image_label)
                ...
                img_loss = self.vis_criterion(
                    ...
                    F.log_softmax(prediction_scores_v, dim=2), image_label
                )
...
class ViLBERT(BaseModel):
    ...
    def get_image_and_text_features(self, sample_list):
        ...
        if sample_list.dataset_name == "nlvr2":
            ...
            image_variable = getattr(sample_list, "image", None)
            # image_label_variable = getattr(sample_list, "image_labels", None)
            image_label_variable = getattr(sample_list, "targets", None)
    ...
    def forward(self, sample_list):
        ...
        if image_feat_variable is not None and params["image_dim"] is not None:
            image_mask = torch.arange(
                image_feat_variable.size(-2), device=image_feat_variable.device
            ).expand(*image_feat_variable.size()[:-1])
