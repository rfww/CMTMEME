...
class VisualBERTBase(BertPreTrainedModel):
    def __init__(
        ...
        # self.polarizer_img = nn.Sequential(
        #     nn.Linear(768, 768),
        #     # nn.ReLU(inplace=True),
        #     # nn.Linear(256, 768)
        # )

        # self.polarizer_txt = nn.Sequential(
        #     nn.Linear(768, 768),
        #     # nn.ReLU(inplace=True),
        #     # nn.Linear(256, 768)
        # )
    ...
    def forward(
        ...
        if (
            ...
            # visual_part = self.polarizer_img(visual_part)
            # sequence_output = self.polarizer_txt(text_embedding_output)
            # embedding_output = torch.cat((sequence_output, visual_part), dim=1)
...
class VisualBERTForClassification(nn.Module):
    def __init__(self, config):
        ...
        self.resnet152 = resnet.resnet152(pretrained=True)  #models_lr
        ...
        self.classifier = nn.Sequential(
            ...
            # nn.Linear(self.bert.config.hidden_size, self.bert.config.hidden_size),  # new added layer
            # nn.Sigmoid(), #  failed all zero
            # nn.Dropout(p=0.4), # I WILL TRY
            nn.Linear(self.bert.config.hidden_size, self.config.num_labels),

        )
        self.pre = nn.Linear(2048, self.config.num_labels)
    ...
    def forward(
        ...
        visual_embeddings = self.resnet152(visual_embeddings)
        #vision prediction
        # B, N, C = visual_embeddings.shape
        # print(F.avg_pool2d(visual_embeddings.permute(0, 2, 1).view(B, C, 10, 10), kernel_size=(10,10)).shape)
        # voutput = self.pre(F.avg_pool2d(visual_embeddings.permute(0, 2, 1).view(B, C, 10, 10), kernel_size=(10,10)).flatten(1))
        ...
        #-------------------------------------------------
        # prob, a = torch.topk(pooled_output, 1, dim=1)
        # print(a)
        # print(pooled_output.shape)
        # pooled_output = F.log_softmax(pooled_output, 1)
        # print(pooled_output)
        #-------------------------------------------------
        logits = self.classifier(pooled_output)
        reshaped_logits = logits.contiguous().view(-1, self.num_labels)
        output_dict["scores"] = reshaped_logits
        # output_dict["voutput"] = voutput
...
class VisualBERT(BaseModel):
    ...
    def build(self):
        ...
        if getattr(self.config, "freeze_base", False):
            ...
            for p in self.model.resnet152.parameters():
                p.requires_grad = False
        ...
        # )
        # self.model.classifier[1][0].requires_grad=True
        # self.model.classifier[1][1].requires_grad=True
        #-----------------------------Add layers------------------------------------- 
    ...
    def update_sample_list_based_on_head(
        ...
        if self.training_head_type == "nlvr2":
            ...
            if not torch.jit.is_scripting():
                ...
                #------------------------------------------------------------
                image_feat = sample_list['image']
                B,C, H, W =image_feat.shape
                image_feat_variable = torch.rand(B,100,2048).cuda()
                sample_list["image_feature_0"] = image_feat_variable
                #------------------------------------------------------------
            else:
